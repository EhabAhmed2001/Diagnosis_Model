...
def helper(dis):
    ...
    Description_result = " ".join([w for w in desc])

    pre = precautions[precautions['Disease'] == dis][['Precaution_1', 'Precaution_2', 'Precaution_3', 'Precaution_4']]
    pre = [col for col in pre.values]
    Precaution_result = ', '.join(pre[0])

    med = medications[medications['Disease'] == dis]['Medication']
    med = [med for med in med.values]
    Medication_split = ', '.join(med)
    Medication_result = str(Medication_split).replace('[', '').replace(']', '').replace("'", '')


    die = diets[diets['Disease'] == dis]['Diet']
    die = [die for die in die.values]
    diet_split = ', '.join(die)
    Diet_result = str(diet_split).replace('[', '').replace(']', '').replace("'", '')


    wrkout = workout[workout['disease'] == dis]['workout']
    Workout_result = ', '.join(wrkout)

    return Description_result, Precaution_result, Medication_result, Diet_result, Workout_result
...
def predict_disease():
    ...
    try:
        ...
        return jsonify({
            ...
            'precautions': Precaution_result,
            'medications': Medication_result,
            'diets': Diet_result,
